...
export default function DeleteJenis({ surat }) {
  ...
  const handleDelete = async () => {
    ...
    const result = await fetch(`/api/jenis/${surat.id}`, {
      method: 'DELETE',
    });
    router.refresh();
    setDelete(false);
          ...
          <div className='bg-black/50 fixed inset-0'>
            <div className=' flex justify-center items-center h-full'>
              <div className='flex flex-col items-center rounded-md bg-slate-300 w-1/3 p-5 pt-1'>
                <h1 className='flex items-end text-2xl py-3'>Delete data</h1>
                <p className='pb-2'>
                  are you sure want to delete this data "{surat.nama}" ?
                </p>
